...
def page_daily_entry():
    ...
    # ════════════════════════════════════════════════════════════
    # TAB 2 – Bulk 2-Month Entry
    # ════════════════════════════════════════════════════════════
    with tab2:
        section_header("📦 Bulk 2-Month Entry (Government Ration Cycle)")
        st.markdown("""
            <div class="info-box">
                ℹ️ Use this section to enter data for two consecutive ration months
                in one step. The app maintains the correct
                Opening → Closing chain automatically.
            </div>
        """, unsafe_allow_html=True)
        ...
        with b_col1:
            m1 = st.selectbox("📅 Month 1", months_list, index=0, key="bm1")
            y1 = st.number_input("Year 1", min_value=2020,
                                  max_value=2035, value=today_ist().year, key="by1")
        with b_col2:
            m2 = st.selectbox("📅 Month 2", months_list, index=1, key="bm2")
            y2 = st.number_input("Year 2", min_value=2020,
                                  max_value=2035, value=today_ist().year, key="by2")
        with b_col3:
            b_card = st.selectbox("🃏 Card Type", CARD_TYPES, key="b_card")

        m1_num = months_list.index(m1) + 1
        m2_num = months_list.index(m2) + 1

        # Entry date = 1st of each month
        date1 = date(int(y1), m1_num, 1)
        date2 = date(int(y2), m2_num, 1)

        opening1 = get_previous_closing(date1, b_card)
        st.markdown(f"""
            <div class="info-box">
                Opening Balance for Month 1 ({m1} {y1}):
                <strong>{format_qty(opening1)}</strong>
            </div>
        """, unsafe_allow_html=True)

        bc1, bc2 = st.columns(2)
        with bc1:
            section_header(f"Month 1 – {m1} {y1}")
            recv1 = st.number_input("Received (Kg)", min_value=0.0,
                                     step=0.5, key="br1", format="%.2f")
            clos1 = st.number_input("Closing (Kg)", min_value=0.0,
                                     step=0.5, key="bc1", format="%.2f")
            total1 = opening1 + recv1
            st.info(f"Total: {format_qty(total1)} | Selling: {format_qty(max(0,total1-clos1))}")

        with bc2:
            # Opening of month 2 = closing of month 1 (preview)
            opening2_preview = clos1
            section_header(f"Month 2 – {m2} {y2}")
            st.markdown(f"Opening (from Month 1 closing): **{format_qty(opening2_preview)}**")
            recv2 = st.number_input("Received (Kg)", min_value=0.0,
                                     step=0.5, key="br2", format="%.2f")
            clos2 = st.number_input("Closing (Kg)", min_value=0.0,
                                     step=0.5, key="bc2", format="%.2f")
            total2 = opening2_preview + recv2
            st.info(f"Total: {format_qty(total2)} | Selling: {format_qty(max(0,total2-clos2))}")

        if st.button("💾 Save Both Months", use_container_width=True):
            res1 = upsert_entry(date1, b_card, opening1, recv1, clos1,
                                f"Bulk entry – {m1} {y1}")
            # Opening of month 2 = actual closing saved for month 1
            res2 = upsert_entry(date2, b_card,
                                res1["closing_kg"], recv2, clos2,
                                f"Bulk entry – {m2} {y2}")
            msgs = []
            if res1["adj_message"]: msgs.append(f"Month 1: {res1['adj_message']}")
            if res2["adj_message"]: msgs.append(f"Month 2: {res2['adj_message']}")
            for m in msgs:
                st.warning(m)
            st.success(
                f"✅ Both months saved!\n\n"
                f"**{m1} {y1}:** Received {format_qty(res1['received_kg'])} | "
                f"Closing {format_qty(res1['closing_kg'])}\n\n"
                f"**{m2} {y2}:** Received {format_qty(res2['received_kg'])} | "
                f"Closing {format_qty(res2['closing_kg'])}"
            )
